...
func Test_regexpReplace(t *testing.T) {
	mod := regexpReplace(`module\s+(?P<name>[\S]+)`, `module github.com/kaigedong/subscan-plugin`, "$name")
	assert.Equal(t, "github.com/kaigedong/subscan-plugin", mod)
...
func Test_modPath(t *testing.T) {
	path, _ := filepath.Abs("")
	assert.Equal(t, "github.com/kaigedong/subscan-plugin/tools/", modPath(path))
}
